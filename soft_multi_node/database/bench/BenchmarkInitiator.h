...
class BenchmarkInitiator {
 public:
  ...
  void InitGAllocator() {
    ...
    conf->cluster_size = config_->GetPartitionNum();
    ...
    std::cout << "create default gallocator " << thread_count_ << std::endl;
  ...
  GAddr InitStorage() {
    GAddr storage_addr = DB_Gnullptr;
    ...
    if (config_->IsMaster()) {
      ...
      std::cout << "size " <<  StorageManager::GetSerializeSize() << std::endl;
      storage_addr = default_gallocator->AlignedMalloc(
          StorageManager::GetSerializeSize());
      std::cout << "register table start" << std::endl;
      this->RegisterTables(storage_addr, schemas);
      std::cout << "register table end" << std::endl;
    } 
    return storage_addr;
  }
 protected:
