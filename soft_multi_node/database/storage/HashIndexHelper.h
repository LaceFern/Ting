...
class BucketHeader : public GAMObject{
 public:
  ...
  bool InsertNode(const IndexKey& key, const GAddr& record_ptr,
    ...
    GAddr prev_node_addr = DB_Gnullptr;
    BucketNode cur_node;
    while (cur_node_addr != DB_Gnullptr) {
    ...
    if (cur_node_addr != DB_Gnullptr) { 
      Items items;
      gallocator->Read(cur_node.items_list_, &items, sizeof(Items));
      if (items.AddRecordPtr(record_ptr)) { 
        ...
        GAddr new_items_addr = gallocator->Malloc(sizeof(Items));
