...
int main(int argc, char **argv) {
  ...
  if (dsm->getMyNodeID() == 0) {
    ...
    printf("Node 0: Initialized data at addr 0x%lx\n", addr.addr);
  }
  
  if (dsm->getMyNodeID() == 1) {
    sleep(2);
    printf("Node 1: Writing to addr (node=%d, addr=0x%lx)\n", addr.nodeID, addr.addr);
    dsm->write(addr, 2, b);
    ...
    uint8_t a[3];
    dsm->write(addr, 3, a);
    printf("Node 2: Write completed\n");
  }
  
  std::cout << "Write miss dirty test completed\n";
