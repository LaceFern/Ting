...
FPGACtl::FPGACtl(uint8_t pci_bus,
    ...
    uint8_t pci_dev = 0;
    uint8_t dev_func = 0;
    resourceFilename = getSysPathBarName(pci_bus, pci_dev, dev_func, 2); 
    ...
    resourceFilename = getSysPathBarName(pci_bus, pci_dev, dev_func, 4); 
    fd = open(resourceFilename.c_str(), O_RDWR);
    if (fd < 0) {
        errorPrint(fmt::format(
    ...
    resourceFilename = getSysPathBarName(pci_bus, pci_dev, dev_func, 0); 
    fd = open(resourceFilename.c_str(), O_RDWR);
    if (fd < 0) {
        errorPrint(fmt::format(
...
void CPUMemCtl::writeTLB(const std::function<void(uint32_t, uint32_t, uint64_t, uint64_t)> &func) {
    const auto &[n_pages, vaddr, parray] = page_table;
    const auto page_size = 2U * 1024 * 1024;
