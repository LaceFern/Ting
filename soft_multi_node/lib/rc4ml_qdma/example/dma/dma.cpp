...
void h2c_benchmark_latency(uint8_t pci_bus) {
    ...
    double average_latency = 1.0 * count_latency * 4 / total_cmds / 1000;
    fmt::println("average_latency	   {:.1f} us", average_latency);
    double ops_limit = 1.0 * 250 * 1024 * 1024 / wait_cycles / 1024 / 1024;
    fmt::println("ops_limit            {:.1f} Mops", ops_limit);
    double ops = 1.0 * total_cmds / (1.0 * cycles * 4 / 1000 / 1000 / 1000) / 1024 / 1024;
...
void c2h_benchmark_latency(uint8_t pci_bus) {
    ...
    double average_latency_cmd = 1.0 * count_latency_cmd * 4 / total_cmds / 1000;
    double average_latency_data = 1.0 * count_latency_data * 4 / total_cmds / 1000;
    double ops_limit = 1.0 * 250 * 1024 * 1024 / wait_cycles / 1024 / 1024;
    double ops = 1.0 * total_cmds / (1.0 * count_time * 4 / 1000 / 1000 / 1000) / 1024 / 1024;
    fmt::println("wait cycles              {}", wait_cycles);
